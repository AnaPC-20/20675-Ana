...
class Dni {
        ...
        static esValidoDNI(dni){  // se le llama Dni.esValidoDNI(dni_completo);
            let letra = dni.slice(dni.length-1);
            let numero = dni.slice(0,dni.length-2);
            let resto =numero %23;
            let resultado = false;
            if( LETRAS_DNI.charAt(resto) == letra){
                resultado = true;
            }
            return true;
        }
